...
def build_roof(x, y, z, width, height, depth, material):
...
def space_emptier(x, y ,z, width, height, depth):
...
def add_interior_features_2_floor(x, y, z, width, height, depth):
...
def add_interior_features(x, y, z, width, height, depth):
...
def build_living_room(x, y, z, width, height, depth):
    # Sofa
    for dz in range(6):
...
def build_bedroom(x, y, z, width, height, depth):
    #carpet
    for dx in range(8):
        for dz in range(6):
...
def build_interior_wall(x, y, z, material, width):
...
def main():
    ...
    default_width, default_depth, default_height = 20, 20, 6

    # Iterate over build regions and choose the one with the lowest distance
    min_distance = float('inf')
    best_build_region = None

    for build_region in score_all_possible_buildregions(heights):
        origin, size, y, distance = build_region

        # Check if the current build region is better than the previous best
        if distance < min_distance:
            min_distance = distance
            best_build_region = build_region

    # Use default dimensions if no suitable build region is found
    if best_build_region:
        origin, size, y, distance = best_build_region
        width, depth, height = size[0], size[1], random.choice([5, 6, 7, 8])
    else:
        width, depth, height = default_width, default_depth, default_height

    # Now you have the optimal dimensions in width, depth, and height
    print("Optimal Build Region:", best_build_region)
    print("Dimensions: width={}, depth={}, height={}".format(width, depth, height))

    # Call your building functions with the determined dimensions
    build_cellar(x, y, z, width, depth, height)
    build_floor(x, y, z, width, depth, stone)  # Adjust material as needed
    build_second_floor(x, y, z, width, depth, stone)  # Adjust material as needed
    build_walls(x, y, z, width, depth, height, stone)  # Adjust material as needed
    build_roof(x, y, z, width, height, depth, stone)  # Adjust material as needed
    build_staircase(x, y, z, height, stone)  # Adjust material as needed
    space_emptier(x, y, z, width, height, depth)
    add_interior_features_2_floor(x, y, z, width, height, depth)
    add_interior_features(x, y, z)
    build_living_room(x, y, z)
